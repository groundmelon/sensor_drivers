...
Camera::Camera(ros::NodeHandle _comm_nh, ros::NodeHandle _param_nh) : node(_comm_nh), pnode(_param_nh)
{
    ...
    {
        on_off[i] = false;
        pnode.param(std::string("exposure_time_us") + char('a' + i), exposure_time_us[i], 1000);
...
bool Camera::initSingleMVDevice(unsigned int id)
{
    ...
    if (use_auto_exposure)
    {
        ...
        settings.cameraSetting.autoControlParameters.exposeUpperLimit_us.write(exposure_time_us[ids_inv[id]]);
