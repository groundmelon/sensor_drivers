...
{
    std::string portstr;
    nh_private.param("devname", portstr, std::string("/dev/ttyUSB0"));
    // portname = portstr.c_str();
    fd = open (portstr.c_str(), O_RDONLY | O_NOCTTY | O_NDELAY);
	...
	pub = nh_private.advertise<geometry_msgs::Vector3Stamped>("height", 10);
	if (fd)
	{
		ROS_INFO("Port <%s> is opened successfully. Ready to read height...\n", portstr.c_str());
	}
	else
	{
		close(fd);
...
float heightFilter::measure(int my_loop_rate)
{
	...
	while (ros::ok())
	{
		...
		if (distance_b != -1)
		{
			...
			// ROS_INFO("Distance: %5.4f cm", distance_b);
			geometry_msgs::Vector3Stamped msg;
			msg.header.stamp = ros::Time::now();
			msg.header.frame_id = "body";
			msg.vector.z = distance_b / 100.0;
			pub.publish(msg);
		}
		ros::spinOnce();
		loop_rate.sleep();
